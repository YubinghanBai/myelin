...
- For each change, emits a JSON **change envelope** (`insert` / `update` / `delete` / `truncate`) with `lsn_hex`, schema/table identity, and column map (see [Payload example](#payload-example)).
...
**`truncate`:** `"op": "truncate"`; **`row`** is `{}` because PostgreSQL does not send row tuples for `TRUNCATE`.
...
| `PG_TABLE` | `schema.table` ensured in publication; simple identifiers only (default `public.events`) |
| `PGADMIN_URL` | libpq conn string for DDL/admin (defaults from PG* vars) |
| `MYELIN_SKIP_SCHEMA` | If truthy (`1`, `true`, `yes`, `on`), skip applying `schema/events.sql` |
| `NATS_URL` | If set, enable JetStream publisher |
| `NATS_STREAM`, `NATS_SUBJECT_PREFIX` | JetStream stream name / subject prefix; prefix is normalized without trailing dots |
| `MYELIN_MAX_PAYLOAD_BYTES` | Max serialized envelope size (default 768KiB) |
| `MYELIN_OVERSIZED_POLICY` | `stall` (default) or `dead_letter` |
| `MYELIN_DLQ_SUBJECT` | Dead-letter subject when policy is `dead_letter`; must be under `NATS_SUBJECT_PREFIX` |
| `MYELIN_LOG_ENVELOPE` | If truthy, log each JetStream publish (testing / debug) |
...
| `myelin_replication_last_applied_lsn_raw` | gauge | Last LSN reported to Postgres as applied |
| `myelin_replication_last_applied_lsn_by_source_raw` | gauge `{source}` | Applied LSN split by `xlog_data` vs `commit` |
| `myelin_replication_applied_lsn_updates_total` | counter `{source}` | Number of applied-LSN progress reports |
...
| `myelin_envelopes_per_xlog_chunk` | histogram | Envelope count per WAL payload |
| `myelin_envelope_json_bytes` | histogram | Serialized JSON envelope size |
| `myelin_jetstream_publish_ack_total` | counter `{op}` | JetStream PubAck after publish |
| `myelin_oversize_dlq_total` | counter | Oversized rows successfully PubAcked as DLQ notices |
| `myelin_logging_envelope_total` | counter `{op}` | Log-only publisher envelopes |
